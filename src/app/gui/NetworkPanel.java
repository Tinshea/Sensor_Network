 ...
 * visualization, including nodes, connections, and light animations. It implements the
 * {@link GraphicalNetworkInterface} to provide a unified interface for managing graphical
...
public class NetworkPanel extends JPanel implements GraphicalNetworkInterface {
    ...
    private void drawGrid(Graphics2D g2d) {
        ...
        for (int i = gridSize; i < height; i += gridSize) {
            ...
            g2d.drawString(String.valueOf((height - i) / gridSize), 5, i + (g2d.getFontMetrics().getAscent() - g2d.getFontMetrics().getDescent()) / 2);
        }

        for (int i = gridSize; i < width; i += gridSize) {
            g2d.drawLine(i, 0, i, height);
            g2d.drawString(String.valueOf(i / gridSize), i - g2d.getFontMetrics().stringWidth(String.valueOf(i / gridSize)) / 2, height - 5);
        }
    }
    
    /**
     * Paints all components of the network including nodes, connections, and lights.
     * This method is overridden to ensure custom drawing of all graphical elements.
    ...
    /**
     * Adds a node to the network visualization at specified grid coordinates, converting these
     * coordinates into pixel positions for graphical display. The node is centered based on its
     * diameter to ensure precise placement on the grid.
     *
    ...
    /**
     * Creates a connection between two nodes identified by their names. If both nodes exist,
     * a new connection is established and the graphical representation is updated.
     *
    ...
    /**
     * Implements the {@link GraphicalNetworkInterface} to add a graphical node at specified
     * coordinates. This method is a direct interface for the internal `addNode` method, facilitating
     * the addition of nodes through graphical interactions.
     *
    ...
    /**
     * Implements the {@link GraphicalNetworkInterface} to create a connection between two nodes
     * based on their names. This method provides an accessible interface for adding connections
    ...
    /**
     * Initiates a graphical animation of a light moving between two nodes specified by their names.
     * This method checks for the existence of both start and end nodes and starts the animation
     * if both are found.
     *
    ...
    /**
     * Toggles the blinking state of a node identified by its name to 'true', indicating it should
     * start blinking. This method finds the node and if it exists, toggles its blinking state.
     *
    ...
    /**
     * Resets the blinking state of all nodes in the panel to 'false', stopping any blinking.
     * This method iterates through all nodes and disables their blinking state.
     */
