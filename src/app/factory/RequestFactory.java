...
/**
 * Provides a static factory method for creating different types of query requests
 * for sensor network operations. Each request is tailored to specific sensor data
 * gathering, controlling, or querying tasks using various components of the
 * abstract syntax tree (AST) related to sensor network configurations.
 */
public abstract class RequestFactory {

    /**
     * Enumerates possible types of requests that can be created, ranging from simple
     * data gathering to complex querying with specific control and data operations.
     */
    public enum RequestType {
        ...
        BQUERY_COMPLEX,
        ECONT_WITH_RGATHER,
        FCONT_WITH_MULTIPLE_SENSORS,
        DCONT_COMPLEX,
        BQUERY_DYNAMIC,
        CUSTOM_COMPLEX_QUERY,
        BQUERY_OR,
        BQUERY_AND_VERGLAS,
        BQUERY_AND_NOT_VERGLAS,
        BQUERY_VERIFY_HUMIDITY,
        VERIF_TEMP_SEUIL
    }

    /**
     * Creates a request of the specified type with provided parameters.
     * This method serves as a centralized point to generate different kinds of
     * requests based on AST components and sensor network configurations.
     *
     * @param params Varargs parameter to pass required details like request type, sensor identifiers,
     *               positions, directions, and other necessary parameters based on the request type.
     * @return A {@link RequestI} object configured as per the specified parameters.
     * @throws IllegalArgumentException if the request type is unknown or parameters are incorrect.
     */
    public static RequestI createRequest( Object... params) {
        ...
        switch (type) {
            ...
            case FCONT_BASED:
                ...
                return new Request(new GQuery(new FGather(sensorIdFCont.name()), new FCont(new ABase(position), threshold)), null);
            case DCONT:
                ...
                return new Request(new GQuery(new FGather(sensorIdDCont.name()), new DCont(new Rdirs(direction, new Fdirs(Direction.NW)), depth)), null);
            case BQUERY_COMPLEX:
                ...
                return new Request(new BQuery(
                        new AndBExp(
                                new CExpBExp(new GeqCExp(new SRand(sensorIdHeat.name()), new CRand(heatLevel))),
            ...
            case GQUERY_COMPLEX:
                ...
                return new Request(new GQuery(new RGather(sensorIdComplex1.name(),
                        new RGather(sensorIdComplex2.name(),
                                new FGather(sensorIdComplex3.name()))),
            ...
            case BQUERY_OR:
                ...
                return new Request((QueryI) new BQuery(
                        new OrBExp(
                                new CExpBExp(new GCExp(new SRand(type1.name()), new CRand(valeurPlusGrandeQueSensor))),
            ...
            case ECONT_WITH_RGATHER:
                ...
                return new Request(new GQuery(
                        new RGather(primarySensor.name(), new FGather(secondarySensor.name())),
                        new ECont()), null);
            ...
            case FCONT_WITH_MULTIPLE_SENSORS:
                ...
                return new Request(new GQuery(
                        new RGather(sensor1.name(), new FGather(sensor2.name())),
            ...
            case DCONT_COMPLEX:
                ...
                return new Request(new BQuery(
                        new OrBExp(
                                new CExpBExp(new LeqCExp(new SRand(complexSensor1.name()), new CRand(50.0))), // a montrer au prof
            ...
            case BQUERY_DYNAMIC:
                ...
                return new Request(new BQuery(
                        new CExpBExp(new GeqCExp(new SRand(dynamicSensor.name()), new CRand(criticalLevel))),
                        new DCont(new Rdirs(dynamicDir, new Fdirs(Direction.NW)), 3)), null);
            ...
            case CUSTOM_COMPLEX_QUERY:
                ...
                return new Request(new GQuery(
                        new RGather(customSensor1.name(), new FGather(customSensor2.name())),
            ...
            case VERIF_TEMP_SEUIL:
                ...
                return new Request(new BQuery(new CExpBExp(new LCExp(new SRand(toVerify.name()),new CRand(seuil))),new FCont(new ABase(position4),distanceMax)),null);
            case BQUERY_AND_VERGLAS:
                Position p = (Position) params[1];
                int distanceHumidite = (Integer) params[2];
                return new Request(new BQuery(
                        new AndBExp(
                                new CExpBExp(new LeqCExp(new SRand(EnumSensorIdentifier.Heat.name()), new CRand(273.15))), // a montrer au prof
                                new CExpBExp(new GeqCExp(new SRand(EnumSensorIdentifier.Humidity.name()), new CRand(60.0)))
                        ),
            ...
            case BQUERY_AND_NOT_VERGLAS:
                ...
                return new Request(new BQuery(
                        new AndBExp(
                                new NotBExp(new CExpBExp(new GCExp(new SRand(EnumSensorIdentifier.Heat.name()), new CRand(273.15)))), // a montrer au prof
                                new CExpBExp(new GeqCExp(new SRand(EnumSensorIdentifier.Humidity.name()), new CRand(60.0)))
                        ),
            ...
            case BQUERY_VERIFY_HUMIDITY:
                ...
                return new Request(new BQuery(new CExpBExp(new LCExp(new SRand(EnumSensorIdentifier.Humidity.name()),new CRand(seuilHumidity))),new FCont(new ABase(p3),distanceHumidite3)),null);
